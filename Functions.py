...
#Run on MC Step
def step(inflist, infrate, lattice, size, trace, t, wholetime, track):
    check =False
    if (inflist.size > 1):            
        ...
        check = True
        print ('Dead')
    return inflist, trace, t, check, wholetime, track
#Run one Sim
def cycle(inflist, infrate, lattice, size, duration):
    ...
    t=0
    while t < duration:
        inflist, trace, t, stopper, wholetime, track =step(inflist, infrate, lattice, size , trace, t, wholetime, track)
        if stopper == True:
